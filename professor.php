...
if(!$con){
	...
	$result=mysqli_query($con,"SELECT nomeAd,senha FROM professor WHERE nomeAd = '$nomeP' AND senha = '$senha'");
	
	if($array = mysqli_fetch_array($result)){
		echo"<table class='menu' bgcolor='lightblue' align='center'>";
		echo"<tr><td align='center'>Seja Bem - Vindo, $array[nomeAd]</td></tr>";
		echo"</table><br><br>";
		?>
			<table class="menu" bgcolor="lightblue" align="center">
			<tr><td><a href="cadastrar-aluno.php">Cadastrar novo aluno  -</a></td>
			<td><a href="alterar-aluno.php">  Alterar aluno  -</a></td>
			<td><a href="lancar-notas.php"> Lançar notas  -</a></td>
			<td><a href="alterar-notas.php">  Alterar notas</a></td></tr>
			</table>
		<?php
	}else{
		echo("<script type='text/javascript'> alert('nome e/ou senha não encontrado !!'); location.href='professor.html';</script>");
	}
}
?>
...
<table class="menu" bgcolor="lightblue" align="center">
<tr><td><a href="professor.html"> Logout</a></td></tr>
</table>
